...
```
$$
`r eq__(ant_lm2, use_coefs = TRUE, coef_digits = c(2, 2, 2, 2, 2, 2), origdata = ant)`
$$

La longueur de webers peut être estimée à partir de certaines mesures morphologique comme la largeur du protonum, le longueur du fémur postérieure, la longueur de la tête. La relation démontre que la présence ou non de dard influence également cette longueur de webers. Nos graphiques semble monter qu'il y a une différence entre les fourmis possédant des dards ou non. Cette relation est confirmée par notre modèle linéaire qui est significatif au seuil de 5 %. Quand au R^2 ajusté, il décrit 93,54% de la variation du modèle. L'analyse des résidus nous montre également qu'il y a une répartition relativement linéaire des résidus et que leur valeur est moyennement faible par rapport aux valeurs prédites (A). Les résidus ont une distribution qui ne diffère pas trop de la Normale, et elle est symétrique (B). Nous observons également une bonne homocédatiscité des résidus bien qu'elle ne soit pas parfaite (C). Un résidus possède un effet de levier important et un effet de Cook moyen, un autre point possède un effet de Cook important mais ne devrais pas poser de problème au vu de la résistance de notre graphique (D).
